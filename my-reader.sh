...
with serial.Serial('/dev/serial0', 2400, timeout=5) as ser:
    while True:
        s = b''
# kalibrering så den starter med at samle data op lige en det sendes 
# Det kan være et tilfælde, men min måler sender ca. på sekund tal 0, 10, 20 osv
# dette gør også at jeg kan saenke timeout fra 11 til 5 sekunder
        t = datetime.datetime.now()
        koersekund = (t.second + 10) - (t.second % 10) - 1
        future = datetime.datetime(t.year,t.month,t.day,t.hour,t.minute, koersekund)
        ...
        while len(s) != 491:
            try:
                ...
                print('Did not recieve 491 bytes')
            #print('.' + s.hex())
